...
const divNuevosProductos = (producto) => {
  return `
...
const divMasVendidos = (producto) => {
  return `
    <div class="col-span-1 w-full rounded-xl shadow-sm cursor-pointer">
            <div class="bg-gray-100 rounded-t-xl overflow-hidden">
              <img
                src="${producto.imagen}"
                alt="${producto.nombre}"
                class="w-full h-54 object-cover md:object-fill"
...
const cargarProductos = async () => {
  try {
    ...
    nuevosProductos.forEach((producto) => {
      contenedorNP.innerHTML += divNuevosProductos(producto);
    });

    const contenedorMV = document.getElementById("masVendidos");

    masVendido.forEach((producto) => {
      contenedorMV.innerHTML += divMasVendidos(producto);
    });
  } catch (error) {
